          ...
          async function registerSlashCommands(client) {
            ...
            const commands = [
              ...
              new SlashCommandBuilder().setName("level").setDescription("レベル管理")
                .addSubcommand(sub => sub.setName("check").setDescription("レベルを確認").addUserOption(opt => opt.setName("target").setDescription("対象ユーザー").setRequired(false)))
                .addSubcommand(sub => sub.setName("set").setDescription("レベルを設定").addUserOption(opt => opt.setName("target").setDescription("対象ユーザー").setRequired(true)).addIntegerOption(opt => opt.setName("level").setDescription("設定レベル").setRequired(true)))
                .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),
              new SlashCommandBuilder().setName("天気").setDescription("天気を表示／場所を保存").addStringOption(o => o.setName("場所").setDescription("例: 東京、大阪、札幌...").setRequired(false)),
              new SlashCommandBuilder().setName("クイズ").setDescription("クイズを出題").addStringOption(o => o.setName("カテゴリ").setDescription("general / trivia / railway / mix(既定)").setRequired(false)),
              ...
              verifyCommand.data,
              ...panelCommand.data,
              new SlashCommandBuilder().setName("ticket").setDescription("チケットを作成")
            ];
            ...
            client.on("interactionCreate", async (interaction) => {
              try {
                if (interaction.isChatInputCommand()) {
                  ...
                  if (name === "level") {
                    ...
                    if (sub === "check") {
                      ...
                      return interaction.reply({ embeds:[{ color:0x00aaff, title:`${target.username} のレベル`, fields:[{name:"レベル",value:`${level}`,inline:true},{name:"XP",value:`${xp}`,inline:true},{name:"次のレベルまで",value:req?`${req-xp} XP`:"最大"}] }] });
                    }
                    if (sub === "set") {
                      ...
                      return interaction.reply(`✅ ${target.username} のレベルを ${lv} に設定`);
                    }
                  }
                  ...
                  if (name === "join") {
                    if (!interaction.member?.voice?.channel) return interaction.reply({ content:"⚠️ VCに入ってください", ephemeral:true });
                    await interaction.deferReply();
                  ...
                  if (name === "backup") {
                    await interaction.deferReply();
                    if (!hasManageGuildPermission(interaction.member)) return interaction.editReply("⚠️ 権限不足");
                  ...
                  if (name === "addrole") {
                    ...
                    return interaction.editReply(result.success?`✅ ${result.count}人に付与`:`❌ 失敗: ${result.error}`);
                  }
                  ...
                  if (name === "lock") {
                    ...
                    return interaction.editReply(`✅ ロック: ${result.locked}件 / 維持: ${result.unlocked}件`);
                  }
                ...
                if(!interaction.replied) try { await interaction.reply({ content:"❌ エラーが発生", ephemeral:true }); } catch{}
              }
            });
          }
