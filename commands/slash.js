...
const { hasManageGuildPermission, backupServer, restoreServer, nukeChannel, clearMessages, addRoleToAll, lockChannels } = require("../utils/guild");
...
const { inviteCommand, handleInvite, handleGuildMemberAdd } = require("./invite");
...
const aiCooldownExemptIds = ["1401303406596853785","1366740571707801610","1409820488301023257"];
...
async function registerSlashCommands(client) {
  const commands = [
    ...
    new SlashCommandBuilder()
      ...
      .addSubcommand(sub =>
        sub.setName("check").setDescription("レベルを確認").addUserOption(opt => opt.setName("target").setDescription("対象ユーザー").setRequired(false))
      )
      .addSubcommand(sub =>
        sub.setName("set").setDescription("レベルを設定").addUserOption(opt => opt.setName("target").setDescription("対象ユーザー").setRequired(true)).addIntegerOption(opt => opt.setName("level").setDescription("設定レベル").setRequired(true))
      )
    ...
    new SlashCommandBuilder()
      ...
      .addStringOption(o => o.setName("カテゴリ").setDescription("general / trivia / railway / mix(既定)").setRequired(false)),

    new SlashCommandBuilder().setName("join").setDescription("ボイスチャンネルに参加"),
    new SlashCommandBuilder().setName("play").setDescription("音楽を再生（URLまたは検索語）").addStringOption(o => o.setName("query").setDescription("YouTube/Spotify URLまたは検索語").setRequired(true)),
    ...
    new SlashCommandBuilder().setName("backup").setDescription("サーバー構成をバックアップ").setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),
    new SlashCommandBuilder().setName("restore").setDescription("バックアップから復元").setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),
    new SlashCommandBuilder().setName("nuke").setDescription("チャンネルを再作成").setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),
    ...
    new SlashCommandBuilder().setName("addrole").setDescription("全ユーザーにロールを付与").addStringOption(o => o.setName("role_name").setDescription("ロール名またはID").setRequired(true)).setDefaultMemberPermissions(PermissionFlagsBits.ManageRoles),
    new SlashCommandBuilder().setName("lock").setDescription("「認証」を含まないチャンネルをロック").addRoleOption(o => o.setName("role").setDescription("表示を許可するロール").setRequired(true)).setDefaultMemberPermissions(PermissionFlagsBits.ManageChannels),
    new SlashCommandBuilder().setName("unlock").setDescription("すべてのチャンネルをアンロック").setDefaultMemberPermissions(PermissionFlagsBits.ManageChannels),

    verifyCommand.data,
    ...panelCommand.data,
    new SlashCommandBuilder().setName("ticket").setDescription("チケット作成ボタンを表示"),
    inviteCommand.toJSON(), // 招待コマンド
  ];

  const rest = new REST({ version: "10" }).setToken(TOKEN);
  await rest.put(Routes.applicationCommands(CLIENT_ID), { body: commands.map(c => c.toJSON ? c.toJSON() : c) });
  ...
  client.on("interactionCreate", async interaction => {
    try {
      if (interaction.isChatInputCommand()) {
        ...
        if (!interaction.guild && !["ai","天気"].includes(name)) return interaction.reply({ content: "❌ サーバー内でのみ使用可能", ephemeral: true });

        // --- Invite ---
        if (name === "invite") return handleInvite(interaction);
        ...
        if (name === "level") {
          ...
          if (sub==="set") {
            ...
            await setLevelAndXp(interaction.guild.id, target.id, level);
            return interaction.reply(`✅ ${target.username} のレベルを ${level} に設定`);
          }
        }
        ...
        if (["join","play","stop","leave"].includes(name)) {
          ...
          if (name==="play") {
            ...
            if (!member?.voice?.channel) return interaction.editReply("⚠️ VCに入ってください");
            try {
              const title = await playUrl(interaction.guild.id, query, interaction.channel, member.voice.channel);
              return interaction.editReply(`▶️ 再生開始: **${title}**`);
            } catch (e) {
              console.error(e);
              return interaction.editReply("⚠️ 再生中にエラー発生");
            }
          }
          if (name==="stop") { const conn=getVoiceConnection(interaction.guild.id); if(conn){conn.destroy(); return interaction.editReply("⏹️ 停止");} return interaction.editReply("⚠️ 再生中なし"); }
        ...
        if (["backup","restore","nuke","clear","addrole","lock","unlock"].includes(name)) {
          ...
          if (name==="nuke") { await nukeChannel(interaction.channel); return interaction.editReply("💥 再作成"); }
      ...
      if(!interaction.replied){ try{ await interaction.reply({ content:"❌ エラー発生", ephemeral:true }); }catch{} }
    }
  });
