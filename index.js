...
const { 
  Client, GatewayIntentBits, PermissionsBitField, REST, Routes, SlashCommandBuilder, 
...
if (process.env.SELF_URL) setInterval(() => {
...
async function translateWithRetry(text, options, retries = 3) {
  for (let i = 0; i < retries; i++) {
    ...
    catch (e) { if (e?.name === 'TooManyRequestsError') await delay(1500*(i+1)); else throw e; }
  }
    ...
    if (ch.permissionOverwrites?.cache?.size) {
      ch.permissionOverwrites.cache.forEach(ow=>{
        if(ow.type===0) overwrites.push({
          id: ow.id, allow: ow.allow.bitfield.toString(),
          deny: ow.deny.bitfield.toString(), type: 0
        });
      });
    }
    return {
      ...
      userLimit: ch.userLimit||null, overwrites
    };
  });

  return { 
    meta: { guildId: guild.id, name: guild.name, iconURL: guild.iconURL({ size: 512 })||null, savedAt: new Date().toISOString() }, 
    roles, channels 
  };
}
...
async function restoreGuildFromBackup(guild, backup, interaction){
  ...
  for(const cat of categories){
    try{
      const created = await guild.channels.create({
        ...
        position:cat.position, reason:'Restore: create category'
      });
  ...
  if(interaction){ await interaction.followUp({content:'✅ 完全復元が完了しました',flags:64}); }
}
